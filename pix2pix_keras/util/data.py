...
class TwoImageIterator(Iterator):
    ...
    def _load_img_pair(self, idx, load_from_memory):
        ...
        a = tiff.imread(os.path.join(self.a_dir, fname))
        mean = tiff.imread('data/data/mean_img.tif')
        # std=tiff.imread('data/data/std_img.tif')
        print a.shape
        a = a-mean

        a = np.rollaxis(a, 2, 0)
        b = tiff.imread(os.path.join(self.b_dir, fname))[np.newaxis, :, :]
